...
for (matrix_id = 1; matrix_id <= maximum_number_of_omegas; matrix_id += 1) {
    if (doSynRateVariation) {
        ...
        // XXX new:
        //PopulateModelMatrix			  ("MGMatrix" + matrix_id,  nucCF, "syn" + matrix_id, "omega" + matrix_id, "");    
    } else {
...
if (PRINT_MG94 == 1) {
    mg94csvFilePath = csvFilePath + ".mglocal.csv";
    fprintf(mg94csvFilePath, CLEAR_FILE, KEEP_OPEN, "Branch,Mean_dNdS,BranchLength");
}
...
//
GetInformation(varNames, "givenTree\\..*\\.nonsyn");
localNonSynValues			 = {totalBranchCount,1}["Eval(varNames[_MATRIX_ELEMENT_ROW_])"];
GetInformation(varNames, "givenTree\\..*\\.syn");
localSynValues			 = {totalBranchCount,1}["Eval(varNames[_MATRIX_ELEMENT_ROW_])"];

sortedDnDs = {};
for (k = 0; k < totalBranchCount; k = k + 1) {
    for (j = 0; j < totalBranchCount; j = j + 1) {
        if ((varNames[k]$bNames[j])[0] != -1) {
            sortedDnDs[j] = (localNonSynValues[k])/(localSynValues[k]);
        }
    }
}
...
if (PRINT_MG94 == 1) {
    for		(k = 0; k < totalBranchCount; k = k+1) {
        fprintf (mg94csvFilePath, "\n", bNames[k], ",", sortedDnDs[k], ",", mg94bls[k]);
    }
}
...
if (RUN_BSREL == 1) {
    ...
    for (k = 0; k < totalBranchCount; k+=1) {
        ...
        // XXX HERE, but going to look at copyomegas to see if that is wehre
        // things are setup for synvar vs nonsynvar
        // That is indeed where these things are set up. We'll see what
        // happens...
        if (_useGridSearch) {
            if (!doSynRateVariation) {
...
function copyomegas (node, syn, nonsyn) {
    if (doSynRateVariation) {
        ...
        if (local_omega < 1) {
            ...
            Eval("`node`.syn1=  syn");
            Eval("`node`.omega2 = 10");
            Eval("`node`.syn2=  1"); // not sure what this should be. One works...
        } else {
            Eval("`node`.Paux1 = 0");
            Eval("`node`.omega2 =  local_omega");
            Eval("`node`.syn2=  syn"); // not sure what this should be. One works...
            Eval("`node`.omega1 = 0");
            Eval("`node`.syn1=  1");
        }
        */
