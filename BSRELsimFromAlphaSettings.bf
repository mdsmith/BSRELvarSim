...
for (branch_id = 0; branch_id < Columns(topology_branch_names)-1; branch_id += 1) {
    ...
    // XXX
    assert (Columns (omega_info) == 3, "Not a 3-column matrix with the omega distribution for " + branch_name);
    
    rate_classes   = Rows (omega_info);
    // XXX
    scaled_weights = omega_info[-1][2];
    ...
    for (rate_class = 1; rate_class <= rate_classes; rate_class += 1) {
        ...
        *(full_name + ".syn" + rate_class) = omega_info[rate_class-1][1];
        *("syn" + rate_class) = omega_info[rate_class-1][1];
    ...
    printNodeDesc (full_name, rate_classes, solve_these_for_lengths);
}
...
lfunction printNodeDesc (ref, rate_classes, length_formulae) {
    ...
    for (k = 0; k < rate_classes; k+=1) {
        if (do_srv) {
            ...
            ExecuteCommands("nonsyn" + (k+1) + " = " + nonsyn);
            ExecuteCommands("syn" +(k+1) + " =  " + syn);
            ExecuteCommands("Paux" + (k+1) + " = " + Eval(wts[k]));
            if (syn != 0) {
                    _ratio = nonsyn/syn;
                    if (_ratio < 10000) {
                            omega = Format (_ratio,10,8);
                    }
            } else {
                    if (nonsyn == 0) {
                            omega = " Undefined";
    ...
    ExecuteCommands("node_length = (" + length_formulae[rate_classes] + ")/3");
    fprintf(stdout, "\n\tlength:  ", node_length);
    fprintf (stdout, "\n"); 
    return 0;
}
